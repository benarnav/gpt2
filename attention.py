...
class Attention(nn.Module):
    ...
    def forward(self, residual: torch.Tensor):  # shape ["batch", "seq", "d_model"]
        K = einops.einsum(self.W_K, residual, "num_heads d_model d_head, batch seq d_model -> batch seq num_heads d_head")
        K += self.b_K

        Q = einops.einsum(self.W_Q, residual, "num_heads d_model d_head, batch seq d_model -> batch seq num_heads d_head")
        Q += self.b_Q

        V = einops.einsum(self.W_V, residual, "num_heads d_model d_head, batch seq d_model -> batch seq num_heads d_head")
        V += self.b_V

        QK = einops.einsum(Q, K, "batch seq_q num_heads d_head, batch seq_k num_heads d_head -> batch num_heads seq_q seq_k")
        QK_masked = QK + self.mask

        soft = torch.softmax((QK_masked / math.sqrt(self.config.d_head)), dim=-1)
        print(f"{soft.shape=}")
        attn = einops.einsum(
            soft, V, "batch num_heads seq_q seq_k, batch seq_v num_heads d_head -> batch seq_q num_heads d_head"
        )
        multi_head = einops.rearrange(attn, "batch seq_q num_heads d_head -> batch seq_q (num_heads d_head)")

        return (
            einops.einsum(
                self.W_O, multi_head, "num_heads_d_head d_model, batch seq_q num_heads_d_head -> batch seq_q d_model"
            )
            + self.b_O
        )
