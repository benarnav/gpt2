...
class Attention(nn.Module):
    ...
    def forward(self, residual: float[torch.Tensor, "batch seq d_model"]):  # shape ["batch", "seq", "d_model"]
        ...
        Q = einops.einsum(
            self.W_Q, residual, "num_heads d_model d_head, batch seq d_model -> batch seq num_heads d_head"
        )
        Q += self.b_Q

        V = einops.einsum(
            self.W_V, residual, "num_heads d_model d_head, batch seq d_model -> batch seq num_heads d_head"
        )
        V += self.b_V

        QK = einops.einsum(
            Q, K, "batch seq_q num_heads d_head, batch seq_k num_heads d_head -> batch num_heads seq_q seq_k"
        )
